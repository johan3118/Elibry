...
import { Save, Users, ArrowLeft, AlertCircle, Plus, Trash2 } from "lucide-react"
import { useRouter, useSearchParams } from "next/navigation"
import { supabase, uploadImage, type Cliente } from "@/lib/supabase"
import { actualizarClienteAction } from "@/app/actions/crm-actions"
...
export default function EditarClientePage() {
  ...
  const cargarCliente = async () => {
    try {
      ...
      const emailList = data.email ? data.email.split(",").map((e: string) => e.trim()).filter(Boolean) : [""]
      setEmails(emailList.length > 0 ? emailList : [""])
      
      // Set country
      setPais(data.pais || "República Dominicana")
      
      // Load existing documents
      setExistingDocUrls((data as any).documentos_urls || [])
      
      // Detect ID type from existing data
      if (data.tipo_cliente === "NORMAL" && data.identificacion) {
        const id = data.identificacion
        if (/^\d{3}-\d{7}-\d{1}$/.test(id)) {
  ...
  const validateForm = () => {
    ...
    if (!formData.tipo_cliente) {
      newErrors.tipo_cliente = "Debe seleccionar el tipo de cliente"
    }
    ...
    } else if (formData.tipo_cliente === "NORMAL") {
    ...
    if (validEmails.length === 0) {
      ...
      for (const email of validEmails) {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
          newErrors.email = `El formato del email "${email}" no es válido`
          break
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imagenUrl = cliente?.imagen_url

      // Subir nueva imagen solo si es empresa y se seleccionó una
      if (formData.tipo_cliente === "EMPRESA" && selectedImage) {
        try {
      ...
      // Subir documentos adjuntos nuevos y combinar con los existentes no eliminados
      ...
      const clienteData = {
        ...
        nombre_comercial: formData.tipo_cliente === "EMPRESA" ? formData.nombre_comercial : null,
        responsable: formData.tipo_cliente === "EMPRESA" ? formData.responsable : null,
        // Campos para cliente normal
        identificacion: formData.tipo_cliente === "NORMAL" ? formData.identificacion : null,
        nombre_completo: formData.tipo_cliente === "NORMAL" ? formData.nombre_completo : null,
        sexo: formData.tipo_cliente === "NORMAL" ? formData.sexo : null,
        fecha_nacimiento:
          formData.tipo_cliente === "NORMAL" && formData.fecha_nacimiento ? formData.fecha_nacimiento : null,
        // Campos comunes
      ...
      const estadoRegistro = isAdmin ? "PERMANENTE" : "MODIFICADO"

      const result = await actualizarClienteAction(Number(clienteId), {
        ...clienteData,
      ...
      if (isAdmin) {
        ...
        alert("Cliente actualizado. Los cambios estan pendientes de aprobacion del administrador.")
      }
  ...
  const esProvisional = cliente.estado_registro !== "PERMANENTE"
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-blue-600">Editar Cliente</h1>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {errors.tipo_cliente && <p className="text-red-500 text-sm">{errors.tipo_cliente}</p>}
                </div>
                <div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-blue-600">
                  {formData.tipo_cliente === "EMPRESA" ? "Información de la Empresa" : "Información Personal"}
                </CardTitle>
                <CardDescription>
                  {formData.tipo_cliente === "EMPRESA" ? "Datos de la empresa" : "Datos personales del cliente"}
                </CardDescription>
              </CardHeader>
                  ...
                  <>
                    ...
                    <div className="pt-4 border-t">
                      <ImageUpload
                        ...
                        currentImage={cliente?.imagen_url}
                        tipoCliente="EMPRESA"
                        disabled={loading}
                      />
                    </div>
