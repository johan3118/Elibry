...
describe("buildVoucherData — block-never-default, one test per branch", () => {
  const cases: Array<{ name: string; break_: (i: BuildVoucherDataInput) => void; expectedMissing: string }> = [
    ...
    { name: "telefonoHotel missing (productos.telefonos_json)", break_: (i) => { i.telefonoHotel = undefined }, expectedMissing: "TELEFONO (productos.telefonos_json)" },
    { name: "telefonoHotel blank string still blocks — never falls back to \"\", the supplier's phone, or the agency's phone", break_: (i) => { i.telefonoHotel = "   " }, expectedMissing: "TELEFONO (productos.telefonos_json)" },
  ...
  it("collects EVERY missing field in ONE pass — not short-circuited on the first broken field", () => {
    ...
    expect(result.missing).toContain("TELEFONO (productos.telefonos_json)")
...
// `productos.telefonos_json` (the hotel PROPERTY's own front-desk number(s),
// joined by the caller before reaching this builder), never
// `suplidores.telefono` (a column that never existed), never the dead
// `productos.telefono_contacto` column, and never a fallback to "", the
// supplier's, or the agency's phone. End-to-end: a valid telefonos_json-
// shaped value flows verbatim from the builder into the RENDERED voucher
// HTML; a missing one BLOCKS with the correctly-named label instead of
// silently defaulting.
// ─────────────────────────────────────────────────────────────────────────────
describe("buildVoucherData — TELEFONO sourced from productos.telefonos_json (HOTFIX 2026-07-28)", () => {
  it("a valid productos.telefonos_json-shaped value flows verbatim through buildVoucherData AND into the rendered voucher HTML", async () => {
    const { generateVoucherDocHTML } = await import("../lib/document-generator")

    const input = clone(validInput())
    input.telefonoHotel = "(809) 552-1444" // shape of a real productos.telefonos_json-derived value
  ...
  it("a missing telefonoHotel BLOCKS with the productos.telefonos_json label and NEVER reaches the renderer", () => {
    const input = clone(validInput())
    input.telefonoHotel = undefined
    const result = buildVoucherData(input)
    ...
    expect(result.missing).not.toContain("TELEFONO (productos.telefono_contacto)")
  })
})
